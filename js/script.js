...
info.update = function(props) {
    this._div.innerHTML = '<h5>Neighborhood:</h5>' + (props ?
        '<b><div id="offset">' + props.NHOOD + '</b></div>' +
        '<h5>Auctions:</h5><b><div id="offset">' + props.Count_ +
        ' homes</b></div>' :
...
function getColor(x) {
    return x > 50000 ? "#005a32" : x > 25000 ? "#238b45" : x > 10000 ? "#41ab5d" :
        x > 5000 ? "#74c476" : x > 1000 ? "#a1d99b" : "#c7e9c0";
}

function getFontColor(z) {
    return z > 10000.1 ? "white" : "#191919";
}

function getRadius(r) {
        return r > 50000 ? "12.5" : r > 25000 ? "11" : r > 10000 ? "10" : r > 5000 ?
            "9" : r > 1000 ? "8.5" : "8";
    }
...
function makeMarkers(feature, layer) {
        ...
        layer.on("click", function(e) {
            var priceFormat = accounting.formatMoney(thisFeature.price, "$", 0, ",",
                "."); //accesses accounting.js to format into currency
            var priceFormatNGBH = accounting.formatMoney(thisFeature.sale_summaries_NGBR_AVG,
                "$", 0, ",", ".");
            ...
            var thisPropertyWidth = Math.log10(thisFeature.price) * Math.log10(
                thisFeature.price) * Math.log10(thisFeature.price) * 3.68;
            $('#thisProperty').text(thisFeature.address).css({
                "width": thisPropertyWidth,
                "background-color": getColor(thisFeature.price),
                "color": getFontColor(thisFeature.price)
            })
            ...
            var neighborhoodWidth = Math.log10(thisFeature.sale_summaries_NGBR_AVG) *
                Math.log10(thisFeature.sale_summaries_NGBR_AVG) * Math.log10(thisFeature.sale_summaries_NGBR_AVG) *
                3.68;
            $('#neighborhoodName').text(thisFeature.MPSUBSEC01).css({
                "width": neighborhoodWidth,
                "background-color": getColor(thisFeature.sale_summaries_NGBR_AVG),
                "color": getFontColor(thisFeature.sale_summaries_NGBR_AVG)
            })
        });
        layer.on({
